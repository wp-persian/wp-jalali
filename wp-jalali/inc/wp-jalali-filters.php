...
// change en number to persian number
...
function ztjalali_pre_get_posts_filter_fn($query) {
    ...
    if (!empty($year) and ! empty($monthnum)) {
        ...
        if ($monthnum == 12 and ! (isKabise($year)))
            $end_date = jalali_to_gregorian($year, $monthnum, 29);
        else
            $end_date = jalali_to_gregorian($year, $monthnum, 30);
